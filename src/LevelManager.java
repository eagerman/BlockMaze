...
public class LevelManager {
	...
	public LevelManager() throws IOException {
		...
		files_dbug();//////
		this.won = new boolean[this.num_levels];
		init_won_levels();
		
	}
	
	private void init_won_levels() {
		
		for(int i=0; i < this.num_levels; i++) this.won[i] = false;
		
	}
	
	public void set_level_as_won() {
		
		System.out.println("Setting level "+this.level_counter+" as finished");
		
		this.won[this.level_counter] = true;
		
		//if(all_won()) then waht?
		
	}
	
	private boolean all_won() {
		
		for(int i=0; i < this.num_levels; i++) if(this.won[i]==false) return false;
	...
	public Level get_next_level() {
		...
		System.out.println("Level counter is "+this.level_counter);
		
		return next;
		
	}
	
	public Level get_prev_level() {
		
		if(this.level_counter == 0) return this.current_level;
